...
var files = (function () {
    ...
    Singleton.defaultOptions = {
        ...
        ----------------------------------------------------------------------------------------------------
        Prompt: 
        The .wav file is a recording of a captured transmission between two HAM radio stations.
        ----------------------------------------------------------------------------------------------------
        The first thing to do when faced with this kind of challenge is to check if it's a .wav file with the command file, which certifies that it's a .wav file. 

        Then, we use exiftool to see if the flag has been hidden in the comments we can add to the file. We notice something interesting:

        Comment: BPSK63 Freq: 143101.500.

        So the transmission was modulated in BPSK63, at the frequency 143101.500Hz. 

        To get our flag, I used the FLDiGi software, which is a software for amateur radio transmission.

        I load the file in the software, set the frequency and the modulation algorithm.
        In the box that shows us the demodulation of the signal, we see the flag appearing progressively: auctf{bigger_number_equals_more_better}.
        `,
        "YouCanTellByThePixels":`
        -------------------------------------------------------------------------------------------------------
        Prompt: 
        The .wav file is a recording of a captured transmission between two HAM radio operators. 
        Given the large bandwidth and length of transmission, it is suspected that this is a file transfer.
        -------------------------------------------------------------------------------------------------------
        In this challenge, we hear about file transmission again. So I decide to reuse exiftool on the .wav file. That's a good idea! Once again, we observe :

        Comment: 8PSK500 freq=433101.500

        So, our transmission is modulated in 8PSK500, at the frequency 433101.500Hz.

        So we load this file in the FLDiGi software, adjust the modulation algorithm and the frequency. A very long chain in base64 starts to appear. We wait for the end of the transmission.

        When this one is finished, I say to myself that this file is surely an image.
        So I use a site which allows to transform the base64 into an image.

        We download the generated file, we open the image.
        We can read: auctf{M5_PANT}.
        `
    };
    return {
        getInstance: function (options) {
...
var main = (function () {
    ...
    var Terminal = function (prompt, cmdLine, output, sidenav, profilePic, user, host, root, outputTimer) {
        ...
        (typeof user === "string" && typeof host === "string") && (this.completePrompt = user + "@" + host + ":~/writeups/auctf2020/signal/" + (root ? "#" : "$"));
