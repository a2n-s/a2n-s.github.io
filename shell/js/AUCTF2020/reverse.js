...
var files = (function () {
    ...
    Singleton.defaultOptions = {
        ...
        -------------------------------------------------------------------------
        I download the file, which in this case is an .apk, an android application. 
        I decompile it with Android Studio, to extract the .java source files.

        I gather all the .java files into one, and I run the command: strings * | grep -i auctf
        We get our flag: auctf{m0b1le_r3v3rs1ng!!}
        `,
        "Mobile1":`
        ---------------------------------------------------------------------------------------------
        Prompt: 
        ...
        ---------------------------------------------------------------------------------------------
        We download the file, which in this case is an .ipa, an IOS application. 
        I know that these files can be directly unzipped: unzip mobile1.ipa.
        So I get the folders of the application, I group them all in the same folder, and I run the command :
        strings * | grep -i auctf.

        We get our flag: auctf{i0s_r3v3rs1ng_1s_1nt3r3st1ng}
        `,
        "Mr.GameAndWatch":`
        ---------------------------------------------------------------------------------------------------
        Prompt:
        My friend is learning some wacky new interpreted language and different hashing algorithms. 
        He's hidden a flag inside this program but I cant find it...

        He told me to connect to challenges.auctf.com 30001 once I figured it out though.
        ---------------------------------------------------------------------------------------------------
        We download the file, which in this case is a .class, a java compiled file. 
        So I use a tool on the internet that will give me back the original java file.

        When we open the file, we notice that there are three checks, so three user entries requested, the functions are called:
        - crack_1
        - crack_2
        - crack_3

        So we're starting to look at the first one. This one is rather simple because it just checks a md5 sum. 
        So we go on crackstation to break the hash, we have our first key: masterchief.

        The second one takes a user input and hashes it with SHA1 and compares it to a build SHA1 of the second key. 
        So, we just have to edit the code an add : System.out.println(decrypt(secret_2,key2)), to get the SHA1 hash of the second key.
        I run the program and i've got what i was looking for : 264212deff89ade15661a59e7b632872d858f2c6. 
        We put this hash into crackstation, we broke the hash and get the original text: princesspeach.

        Great we have our second key. So now, let's reverse the third function:

        This one is a little bit tricky, in fact, it's the same as crack_2 function, but this time, the program will XOR all valours in the secret_3 array.
        But, with our method, we just have to add System.out.println(decrypt(encrypt_3,key_3)).
        We got a SHA256 hash : 5ebb49e499a6613e832e433a2722edd0d2947d56fdb4d684af0f06c631fdf633, which corresponds to solidsnake.

        Perfect, we got our 3 keys, let's connect to challenges.auctf.com on port 30001.
        We enter our three keys, and we got our flag: auctf{If_u_h8_JAVA_and_@SM_try_c_sharp_2922}
        `
    };
    return {
        getInstance: function (options) {
...
var main = (function () {
    ...
    var Terminal = function (prompt, cmdLine, output, sidenav, profilePic, user, host, root, outputTimer) {
        ...
        (typeof user === "string" && typeof host === "string") && (this.completePrompt = user + "@" + host + ":~/writeups/auctf2020/reverse/" + (root ? "#" : "$"));
