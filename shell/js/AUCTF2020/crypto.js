...
var files = (function () {
    ...
    Singleton.defaultOptions = {
        ...
        We notice directly that it's base 64, so we'll decode it, we get the flag: auctf{4LL_y0Ur_B453_R_b3l0Ng_2_uS_124df2sdasv}
        `,
        "TheSalad":`
        -------------------------------------------------------------------------------
        Prompt: 
        Bob and Alice love to go to the local 
        farmer's market to get their groceries. You could call them regulars. 
        They know the farmers so well that they've started sharing secret 
        messages with them. Here's one of the messages they wrote to one lettuce
        farmer.
        vpxoa{eP5o_4_R4mH_pK_1_4421_9952}
        -------------------------------------------------------------------------------

        We notice directly, with the name of the challenge, that we are dealing with a Caesar code. So we go to a site that allows us to decipher the Caesar code.

        The offset is 21, so we get the flag: auctf{jU5t_4_W4rM_uP_1_4421_9952}.
        `,
        "ShiftyShwoozy":`
        -------------------------------------------------------------------------------------------------------------------------------
        Prompt:
        Eve woke up to this message after a really crazy night. Everything is shifted around. Can you figure out what it says?

        You can download the message here:
        https://drive.google.com/file/d/1hshUoTtN5tgVjbpRNDujmBnKOJbJZcbS/view?usp=sharing
        -------------------------------------------------------------------------------------------------------------------------------

        When I downloaded that file, I immediately thought we were dealing with a vigenere cipher. 

        Personally, I use a site that works very well for the figure of vigenere: https://www.mygeocachingprofile.com/codebreaker.vigenerecipher.aspx.

        So we enter a part of the ciphered text inside, we wait a few minutes, and the encryption key comes out: COINCIDENCESMAYHELPYOUSOLVETHISPROBLEM.

        
        So we decipher all the text with this key. 
        In the middle of the text, which is an excerpt from The Great Gaby, we notice: sometimes you have to do things yourself. Replace the spaces with underscores and wrap it in the usual format.

        So we have our flag: auctf{sometimes_you_have_to_do_things_yourself}.
        `
    };
    return {
        getInstance: function (options) {
...
var main = (function () {
    ...
    var Terminal = function (prompt, cmdLine, output, sidenav, profilePic, user, host, root, outputTimer) {
        ...
        (typeof user === "string" && typeof host === "string") && (this.completePrompt = user + "@" + host + ":~/writeups/auctf2020/crypto/" + (root ? "#" : "$"));
