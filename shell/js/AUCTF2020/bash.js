...
var files = (function () {
    ...
    Singleton.defaultOptions = {
        "level1" : "---------------------------------------------------------------------------------------\nLevel 1\n Prompt: Connect to ssh level1@challenges.auctf.com 30000, use the password : aubie.\n---------------------------------------------------------------------------------------\nWe see a file: README\nThe file contains: auctf{W3lcoM3_2_da_C7F}",
        
        "level2" : `-----------------------------------------------------------------------------------------------\nLevel 2\n Prompt: Connect to ssh level2@challenges.auctf.com 30000, use as password the flag for level1.\n-----------------------------------------------------------------------------------------------\nWe see a file called random_dirs.sh, let's see what's in it:\n\n#!/bin/bash\nx=$RANDOM\nbase64 flag.txt > /tmp/$x\nfunction finish{\n\&nbsp;&nbsp;rm /tmp/$x\n}\ntrap finish EXIT\nsleep 15\n\nYou can see directly that the program waits 15 seconds before executing the finish function. So we're dealing with some kind of race condition. 
        Let's run the program:

        base64 permission denied
        
        The uid must therefore be changed at the time of execution. So I wrote the following little code, which allows to change the uid, as well as to launch the program : 
        ...
        We launch it, we just have to go in /tmp and cat the file created. We get : YXVjdGZ7ZzB0dEBfbXV2X2Zhczd9.
        
        We just have to decode it, we get the flag: auctf{g0tt@_muv_fas7}.`,
        
        
        "level3" : `
        ----------------------------------------------------------------------------------------
        Level 3
        Prompt: Connect to ssh level3@challenge.auctf.com, use as password the flag of level2. 
        ----------------------------------------------------------------------------------------
        We see a file named passcodes.sh, let's see what in it:
        ...
        We see that the program ask for a random number, let's run it : echo 2000 | ./passcodes.sh :
        1000
        42123 try again.
        
        When we make an ls -la, we see that passcodes.sh have to be execute as level4, not level3. So to get the flag, i've to launch the program as level4.

        So I wrote a script in bash that will always try the number 2000, until the randomly generated number is also 2000 :


        while true; do echo 2000 | sudo -u level4 ./passcodes.sh; done | grep -i auctf


        After a few minutes, I get the flag: auctf{wut_r_d33z_RaNdom_numz}
        `,
        ...
        ----------------------------------------------------------------------------------------
        Level 4
        Prompt: Connect to ssh level4@challenge.auctf.com, use as password the flag of level3. 
        ----------------------------------------------------------------------------------------
        We see a file named print_file.sh, let's see whats in it :
        ...
        then
            ...
            \v echo "Please enter a file."
            \v echo "./print_file FILENAME"
        fi
        
        So wee see that the program print all the files that are passed to him. Sounds great let's try : ./print_file.sh flag.txt

        flag.txt : Permission denied

        When we make an ls -la, we see that print_file.sh have to be execute as level5, not level4. So I just have to execute print_file.sh as level5 :

        sudo -u level5 ./print_file.sh flag.txt

        We got the flag: auctf{FunKy_P3rm1ssi0nZ}
        `,
        ...
        ----------------------------------------------------------------------------------------
        Level 5
        Prompt: Connect to ssh level5@challenge.auctf.com, use as password the flag of level4. 
        ----------------------------------------------------------------------------------------
        We see a file named portforce.sh, let's see whats in it:
        #!/bin/bash
        ...
        So we understand that the program randomly chooses a number between 1024 and 65500 and then uses the netcat command to listen on that number, which corresponds to the port. 

        First of all, I tell myself that I can do an echo of the last used port thanks to netstat, unfortunately, this command is not installed. 

        So you'll have to bruteforce the port number. So I use the following script:


        for i in {1024..65500}; do nc -w 1 challenges.auctf.com $i; echo $i; done


        We wait a few minutes, the port found by the script is good, but we get:

        flag.txt Permission denied


        So we make an ls -la and we see that the script must be run as root.

        So we relaunch the program on their server: sudo -u root ./portforce.sh

        We launch our script, we wait a few minutes, we get the flag: auctf{n3tc@_purt_$can}
        `
    };
    return {
        getInstance: function (options) {
...
var main = (function () {
    ...
    var Terminal = function (prompt, cmdLine, output, sidenav, profilePic, user, host, root, outputTimer) {
        ...
        (typeof user === "string" && typeof host === "string") && (this.completePrompt = user + "@" + host + ":~/writeups/auctf2020/bash/" + (root ? "#" : "$"));
