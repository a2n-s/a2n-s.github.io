...
var files = (function () {
    ...
    Singleton.defaultOptions = {
        ...
        ---------------------------------------------------------
        At the beginning of this challenge, we notice that the hash is md5. We are also given a salt: 1337.

        First of all, I try to go to dcode md5 to break the hash with the salt. Unfortunately, it didn't work, and dcode didn't succeed in breaking the hash.

        So I choose the bruteforce method, with a wordlist. I use the rockyou wordlist, which is quite basic but efficient for first crypto challenges.
        So the script is as follows: 
        
        # -*- coding: utf-8 -*-
        import hashlib
        f=open('wordlist.txt','r')
        toFind="5eaff45e09bec5222a9cfa9502a4740d"
        salt="1337"
        lines = f.readlines()
        for line in lines:
        &nbsp;&nbsp;toEncode = line.strip()
        &nbsp;&nbsp;hash = hashlib.md5(salt.encode()+toEncode.encode())
        &nbsp;&nbsp;if(hash==toFind):
        &nbsp;&nbsp;&nbsp;&nbsp;print('Found flag: '+toEncode)

        We wait a few seconds, and the hash is broken: treetop, so we have our flag!
        `,
        "Zippy":`
        ---------------------------------------------------------
        Prompt: 
        ...
        NOTE: The flag is NOT in the standard auctf{} format
        ---------------------------------------------------------
        We download the zip associated with the challenge. 
        We try to unzip it, but a password is requested.

        First, I use a python script with the rockyou wordlist to try to break the password. Unfortunately, it doesn't work.

        So I decide to use zip2john, which is a tool associated with john the ripper, which will allow to find the password hash. 

        It works, we recover the password hash.
        So I use after hashcat to break this hash:
        hashcat -a 0 -m 0 zippy.hash big.txt

        Hashcat finds the password after a few seconds: 8297018229.

        So I go unzip zippy.zip, I enter the password, and there surprise: A second zip that also asks for a password!

        So I reuse the previous technique, and as you can expect, I get another zip with another password.

        I repeat this operation until I get to flag.txt.

        We get our flag: y0ud1d17#15789
        `,
        "Manager":`
        -------------------------------------------------------------------------
        Prompt: 
        ...
        NOTE: The flag is NOT in the standard auctf{} format
        -------------------------------------------------------------------------
        When we look at the file extension given with this challenge, we notice it's a keepass file that contains passwords (.kdbx).

        So I run keepass, and load the file. As I expected, it asks me for a password to be able to open the database that contains the passwords, in this case, our flag.

        Also, in the challenge statement, we are told that the password only contains numbers.

        So I run keepass2john, to find the password hash: keepass2john manager.kdbx > manager.hash .

        So I retrieve the password hash.
        So I use hashcat again to break the password: 
        hashcat64 -a 3 -m 13400 manager.hash ?d?d?d?d?d?d?d?d?d

        We get the password: 157865
        So we restart keepass, load the file, enter the password, we have our flag: y0u4r34r34lh4ck3rn0w#!$1678.
        `
    };
    return {
        getInstance: function (options) {
...
var main = (function () {
    ...
    var Terminal = function (prompt, cmdLine, output, sidenav, profilePic, user, host, root, outputTimer) {
        ...
        (typeof user === "string" && typeof host === "string") && (this.completePrompt = user + "@" + host + ":~/writeups/auctf2020/passwdcracking/" + (root ? "#" : "$"));
